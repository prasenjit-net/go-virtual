...
func TestUpsertStoreEntry_Success(t *testing.T) {
	...
	body := `{"value":"hello"}`
	req := httptest.NewRequest("PUT", "/store/greeting", bytes.NewBufferString(body))
	req.Header.Set("Content-Type", "application/json")
	w := httptest.NewRecorder()
	r.ServeHTTP(w, req)
