...
type CompiledScript interface {
	...
	Execute(ctx context.Context, input *ScriptInput, timeoutMs int, sess store.SessionState, accessLog *[]models.StoreAccessEvent, logBuf *[]string, collBackend store.CollectionBackend) (any, error)
}
...
func (s *starlarkScript) Execute(ctx context.Context, input *ScriptInput, timeoutMs int, sess store.SessionState, accessLog *[]models.StoreAccessEvent, logBuf *[]string, collBackend store.CollectionBackend) (result any, err error) {
	...
	if hasSession(sess) {
		predeclared["store"] = store.NewStoreBuiltin(sess, collBackend, accessLog)
	}
