...
`
	...
	result, err := cs.Execute(context.Background(), input, 100, nil, nil, nil, nil)
	if err != nil {
...
func TestExecute_ReturnsString(t *testing.T) {
	...
	result, err := cs.Execute(context.Background(), &ScriptInput{}, 100, nil, nil, nil, nil)
	if err != nil {
		t.Fatalf("Execute: %v", err)
	}
	if result != "hello" {
...
func TestExecute_NoRunFunction(t *testing.T) {
	...
	_, err = cs.Execute(context.Background(), &ScriptInput{}, 100, nil, nil, nil, nil)
	if err == nil {
...
`
	...
	_, err = cs.Execute(context.Background(), &ScriptInput{}, 5, nil, nil, nil, nil)
	if err == nil {
