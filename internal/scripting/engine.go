...
// RunSpecBindings executes all enabled script bindings attached to the given
// spec and returns their merged output. Spec-level scripts run before operation
// scripts to provide a shared base layer.
func (e *ScriptEngine) RunSpecBindings(
	ctx context.Context,
	specID string,
	input *ScriptInput,
	sess store.SessionState,
	...
	bindings, err := e.store.GetSpecScriptBindings(specID)
	if err != nil || len(bindings) == 0 {
		return output, traces
	}
