...
// Recorder proxies requests to an upstream backend and records every unique
// response as a ResponseConfig so it can be replayed when proxy mode is off.
...
func (r *Recorder) ProxyAndRecord(
	...
	go r.recordResponse(operation, signature, statusCode, respHeaders, respBody)

	return statusCode, respHeaders, respBody, nil
}

// recordResponse saves (or updates) the backend response as a ResponseConfig
// for the given operation keyed by the request signature.
func (r *Recorder) recordResponse(
	op *models.Operation,
	signature string,
	statusCode int,
	...
	// Check whether a recorded response for this signature already exists
	...
	if existing != nil {
		// Update the existing entry with the latest backend response
	...
	// Create a new recorded ResponseConfig
	now := time.Now()
	...
	cfg := &models.ResponseConfig{
		...
		OperationID: op.ID,
		Name:        fmt.Sprintf("[Recorded] %s", now.Format("2006-01-02 15:04:05")),
		Description: fmt.Sprintf("Auto-recorded from backend at %s (sig: %s)", now.Format(time.RFC3339), sigPreview),
