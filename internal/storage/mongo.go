...
func (m *MongoStorage) GetResponseScriptBindings(responseConfigID string) ([]*models.ScriptBinding, error) {
	ctx, cancel := ctxTimeout()
	defer cancel()
	cursor, err := m.col(colBindings).Find(ctx, bson.M{"response_config_id": responseConfigID})
	if err != nil {
		return nil, err
	}
	defer cursor.Close(ctx)
...
func (m *MongoStorage) DeleteScriptBindingsByResponse(responseConfigID string) error {
	if responseConfigID == "" {
		return nil
	}
	ctx, cancel := ctxTimeout()
